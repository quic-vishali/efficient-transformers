...
def load_seq2seq_model(model_config):
    ...
    kwargs["torch_dtype"] = model_config.get("torch_dtype", torch.float32)
...
def run_seq2seq_pytorch_hf(
    model, processor: AutoProcessor, inputs: np.ndarray, sample_rate: int, generation_len: int, torch_dtype = torch.float32
) -> List[str]:
    """
    ...
    # prepare inputs
    input_features = processor(inputs, sampling_rate=sample_rate, return_tensors="pt").input_features.to(torch_dtype)
...
def run_seq2seq_ort(
        ...
        for self_cross in ["self", "cross"]:
            for kv in ["key", "value"]:
                ...
                model_inputs[f"past_{kv}_{self_cross}.{i}"] = torch.zeros(
                    kv_cache_shape if self_cross == "self" else kv_cross_cache_shape, dtype=torch_dtype
                )
    ...
    model_inputs["input_features"] = torch.tensor(np.zeros((batch_size, config.num_mel_bins, 1)), dtype=torch_dtype)
...
def check_seq2seq_pytorch_vs_kv_vs_ort_vs_ai100(
    ...
    qeff_model = QEFFAutoModelForSpeechSeq2Seq.from_pretrained(
        model_name, torch_dtype=torch_dtype
    )
    try:
        qeff_model.export()
        ort_tokens = run_seq2seq_ort(
            qeff_model.onnx_path, qeff_model.model.config, processor, data, sample_rate, ctx_len, torch_dtype=torch_dtype
        )
        assert (pytorch_hf_tokens == ort_tokens).all(), "Tokens don't match for pytorch output and ort output"
