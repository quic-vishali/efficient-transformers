...
def check_qwen3_vl_embedding_cpu_vs_ai100_mad_parity(
    model_name, export_compile_only=False, torch_dtype: torch.dtype = torch.float32
):
    ...
    qeff_config.torch_dtype = torch_dtype
    configure_embedding_model_config(
        config=qeff_config,
        num_hidden_layers=num_layers,
        vision_depth=vision_depth,
...
def load_embedding_model(model_name: str, n_layer: int = -1, torch_dtype: torch.dtype = torch.float32):
    """Load a pre-trained embedding model."""
    kwargs = {"attn_implementation": "eager", "trust_remote_code": True, "torch_dtype": torch_dtype}
    if n_layer > 0:
...
def check_embed_pytorch_vs_ort_vs_ai100(
    ...
    pt_model = load_embedding_model(model_name, n_layer, torch_dtype=torch_dtype)
    ...
    try:
        ...
        # QEFF PyTorch output
        qeff_pt_outputs = qeff_model.generate(
            inputs=inputs,
            runtime_ai100=False,
        )
        qeff_pt_embeddings = (
            qeff_pt_outputs
            if pooling
            else qeff_pt_outputs[0]
        )
        mad = torch.mean(
            torch.abs(pt_embeddings - qeff_pt_embeddings)
        )
        assert mad <= 0, (
            f"MAD is too high for PyTorch and "
            f"PyTorch transformed qeff_model: {mad}"
        )
        ...
        mad = torch.mean(
            torch.abs(
                pt_embeddings - torch.tensor(onnx_outputs[0])
            )
        )
        assert mad <= 1e-3, (
            f"MAD is too high for onnx and Pytorch: {mad}"
        )
        ...
        qeff_ai100_embeddings = (
            ai100_output["output"]
            if pooling
            else ai100_output["output"][
                :, : inputs["input_ids"].shape[1], :
            ]
        )
        # Compare ONNX vs AI100
        mad = np.mean(
            np.abs(
                qeff_ai100_embeddings - onnx_outputs[0]
            )
        )
        assert mad <= 1e-2, (
            f"MAD is too high for onnx and AI 100 output: {mad}"
        )
        ...
        if (
            manual_cleanup is not None
            and hasattr(qeff_model, "onnx_path")
            and qeff_model.onnx_path
        ):
