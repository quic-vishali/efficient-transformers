...
def _run_hf_decode(model, prefill_out, position_ids, generation_len=100):
    """Run HF greedy decode for generation_len steps from a prefill output.

    Returns the concatenated generated token IDs as a numpy array of shape (1, generation_len).
    """
    generated_ids = []
    out = prefill_out
...
def _run_qeff_decode(model, prefill_out, position_ids, generation_len):
    """Apply decode transform to model, run generation_len greedy steps from prefill_out.

    Returns (decode_qeff_model, generated_ids) where generated_ids is shape (1, generation_len).
    decode_qeff_model is returned so the caller can compile it next without re-instantiating.
    """
    ...
    for _ in range(generation_len):
        next_token_id = out["logits"][:, -1, :].argmax(-1).reshape(-1, 1)
        generated_ids.append(next_token_id)
        pos = pos.max(1, keepdim=True).values + 1
        out = decode_qeff_model.model(
            input_ids=next_token_id,
            position_ids=pos,
            past_key_values=out["past_key_values"],
        )
    return decode_qeff_model, np.concatenate(generated_ids, axis=1)
...
def _run_qpc_decode(decode_qpc_path, qpc_prefill_out, qeff_inputs_np, config, generation_len, retain_full_kv=True):
    """Run decode QPC on QAIC for generation_len steps starting from prefill KV state.

    retain_full_kv=True: pass RetainedState directly and feed back each step's RetainedState.
    retain_full_kv=False: uses skip_buffers (on-device KV retained without host round-trip).
    Returns generated token IDs as shape (1, generation_len).
    """
    session = QAICInferenceSession(decode_qpc_path)
    session.set_buffers({"logits": np.zeros((1, 1, config.vocab_size), dtype=np.float32)})

    decode_inputs = {
        "input_ids": np.argmax(qpc_prefill_out["logits"]).reshape(1, 1),
        "position_ids": np.max(qeff_inputs_np["position_ids"]).reshape(1, 1) + 1,
    }
    for i in range(config.num_hidden_layers):
        decode_inputs[f"past_key.{i}"] = qpc_prefill_out[f"past_key.{i}_RetainedState"]
        decode_inputs[f"past_value.{i}"] = qpc_prefill_out[f"past_value.{i}_RetainedState"]

    if not retain_full_kv:
        session.skip_buffers(
            [x for x in session.input_names + session.output_names if x.startswith("past_")]
        )

    generated_ids = [decode_inputs["input_ids"][0][0]]
    decode_out = session.run(decode_inputs)
    ...
    for _ in range(generation_len - 1):
        ...
        if retain_full_kv:
            for i in range(config.num_hidden_layers):
                loop_inputs[f"past_key.{i}"] = decode_out[f"past_key.{i}_RetainedState"]
                loop_inputs[f"past_value.{i}"] = decode_out[f"past_value.{i}_RetainedState"]
        generated_ids.append(loop_inputs["input_ids"][0][0])
        decode_out = session.run(loop_inputs)
        pos_id += 1

    return np.array(generated_ids).reshape(1, -1)
...
def _prefix_caching_inference(
    ...
    is_sliding_window = bool(
        getattr(config, "use_sliding_window", getattr(config, "sliding_window", None) is not None)
    )
